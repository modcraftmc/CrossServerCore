    ...
    public RabbitmqConnection build() throws IOException, TimeoutException {
        ...
        ((Recoverable) connection).addRecoveryListener(new RecoveryListener() {
            ...
            public void handleRecovery(Recoverable recoverable) {
                CrossServerCoreProxy.instance.getLogger().warn("RabbitMQ server is back online");
                onHeartbeatSucceeded.run();
            }
            @Override
            public void handleRecoveryStarted(Recoverable recoverable) {
                CrossServerCoreProxy.instance.getLogger().error("RabbitMQ server is unreachable");
